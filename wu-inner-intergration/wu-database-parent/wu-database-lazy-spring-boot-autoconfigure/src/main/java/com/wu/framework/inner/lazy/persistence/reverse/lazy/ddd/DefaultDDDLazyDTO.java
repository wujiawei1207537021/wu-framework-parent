...
public class DefaultDDDLazyDTO extends DefaultAbstractJavaReverseEngineeringMethod {
    ...
    public DefaultDDDLazyDTO(ReverseClassLazyTableEndpoint reverseClassLazyTableEndpoint, LazyOperationConfig.ReverseEngineering reverseEngineering) {
    ...
    public void initClassAnnotationPart() {
        ...
        if (reverseEngineering.isEnableSwagger()) {
            addClassAnnotationPart(String.format("@Schema(title = \"%s\",description = \"%s\")", tableEndpoint.getTableName() + "_command_dto", tableEndpoint.getComment()));
        }
    }
    ...
    public void initClassNamePart() {
        ...
        String assemblerName = String.format("%sDTO", tableEndpoint.getClassName());
    ...
    protected String getPackage() {
        ReverseClassLazyTableEndpoint tableEndpoint = this.getReverseClassLazyTableEndpoint();
        return String.format("package %s;", tableEndpoint.getPackageName() + NormalUsedString.DOT + "application" + NormalUsedString.DOT + "dto") + NormalUsedString.NEWLINE + NormalUsedString.NEWLINE;
    }
    ...
    public void initClassFieldPart() {
        ...
        fieldEndpoints.stream().collect(
                ...
                fieldLazyTableFieldEndpoint -> {
                    ...
                    if (reverseEngineering.isEnableSwagger()) {
                        apiModelPropertyAnnotation = String.format("    @Schema(description =\"%s\",name =\"%s\",example = \"%s\")", comment, fieldLazyTableFieldEndpoint.getName(), "") + NormalUsedString.NEWLINE;
                    }
