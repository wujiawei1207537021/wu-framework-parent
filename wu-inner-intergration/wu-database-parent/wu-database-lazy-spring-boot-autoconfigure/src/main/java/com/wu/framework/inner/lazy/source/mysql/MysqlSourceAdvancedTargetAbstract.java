...
@Slf4j
public abstract class MysqlSourceAdvancedTargetAbstract extends SourceAdvancedAbstract implements SourceAdvanced {
    ...
    public LazyTableEndpoint analyzeLazyTableFromClass(Class<?> clazz) {
        if (!MYSQL_TABLE_CLASS_ATTR_MAP.containsKey(clazz)) {
            synchronized (MYSQL_TABLE_CLASS_ATTR_MAP) {
                LazyTable lazyTable = AnnotatedElementUtils.findMergedAnnotation(clazz, LazyTable.class);
                ...
                JavaClass2MysqlTableLazyTableEndpoint lazyTableEndpoint = new JavaClass2MysqlTableLazyTableEndpoint();
                if (null != lazyTable) {
        ...
        return MYSQL_TABLE_CLASS_ATTR_MAP.get(clazz);
    }
...
//            }
            ...
            LazyTableFieldEndpoint lazyTableFieldEndpoint = new JavaField2MysqlColumnEndpoint();
