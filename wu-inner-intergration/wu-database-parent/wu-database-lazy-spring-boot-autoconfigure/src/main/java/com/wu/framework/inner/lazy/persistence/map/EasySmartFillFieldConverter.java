...
public class EasySmartFillFieldConverter extends EasySmartFillFieldConverterAbstract implements IEasySmartConverter {
    ...
    public String targetClassWriteAttributeFieldList(CreateInfo createInfo) {
        ...
        try {
            BufferedWriter bufferedWriter = FileUtil.createFileBufferedWriter(null, "", createInfo.getFileSuffix(), resourceFile);
...
//            return classContext;
    ...
    /**
     * @param createInfo
     * @return describe 将CreateInfo 转换成String
     * @author Jia wei Wu
    ...
    public String createInfo2String(CreateInfo createInfo) {
        ...
        // innerClass
        StringBuilder classContext = new StringBuilder(classPackage + String.join(NormalUsedString.NEWLINE, importClassList) + NormalUsedString.NEWLINE + classname + String.join(NormalUsedString.NEWLINE, attributeFileList));

        for (CreateInfo innerClass : createInfo.getInnerClassList()) {
            classContext.append(innerClass2String(innerClass));
        }
    ...
    public String innerClass2String(CreateInfo innerClass) {
        ...
        List<String> attributeFileList = createFieldList.stream().map(createField -> String.format("private %s %s;", createField.getFieldTypeName(), createField.getFieldName())).collect(Collectors.toList());
        StringBuilder first = new StringBuilder(classname + NormalUsedString.NEWLINE + String.join(NormalUsedString.NEWLINE, attributeFileList) + NormalUsedString.NEWLINE + NormalUsedString.RIGHT_BRACE);
        for (CreateInfo createInfo : innerClass.getInnerClassList()) {
            first.append(innerClass2String(createInfo));
        }
