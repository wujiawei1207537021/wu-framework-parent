...
public class SQLConverter {
    ...
    public static String creatTableSQL(Class clazz) {
        ...
        return lazyTableEndpoint.creatTableSQL();
    }
    ...
    public static String updatePreparedStatementSQL(Object o) {
        ...
        for (LazyTableFieldEndpoint convertedField : convertedFieldList) {
            try {
                ...
                Object fieldVal = field.get(o);
                if (Arrays.stream(convertedField.getLazyTableIndexEndpoints()).
                        anyMatch(lazyTableIndexEndpoint -> lazyTableIndexEndpoint.getFieldIndexType().equals(LayerField.LayerFieldType.ID))
                ) {
                    if (punctuationFlag) {
                        stringBuffer.append(",");
                    }
...
//            }
    ...
    /**
     * 根据class 创建mybatis resultMap
     *
     * @param clazz
     */
    public static String createMybatisResultMap(Class clazz) {
//            <resultMap id="BaseResultMap" type="com.xx">
//        <id column="id" property="id" />
//          <result column="code" property="code" />
//         </resultMap>
        StringBuilder resultMap = new StringBuilder(String.format("<resultMap id=\"BaseResultMap\" type=\"%s\">", clazz.getName()));

        for (Field declaredField : clazz.getDeclaredFields()) {
            resultMap.append(NormalUsedString.NEWLINE);
            String name = declaredField.getName();
            if ("id".equals(name)) {
                resultMap.append("<id column=\"id\" property=\"id\" />");
            } else {
                resultMap.append(String.format("<result column=\"%s\" property=\"%s\" />", CamelAndUnderLineConverter.humpToLine2(name), name));
            }
        }
        resultMap.append(NormalUsedString.NEWLINE);
