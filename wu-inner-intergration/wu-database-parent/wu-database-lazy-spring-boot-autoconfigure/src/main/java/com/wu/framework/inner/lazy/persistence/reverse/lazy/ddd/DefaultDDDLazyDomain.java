...
public class DefaultDDDLazyDomain extends DefaultAbstractJavaReverseEngineeringMethod {
    ...
    public DefaultDDDLazyDomain(ReverseClassLazyTableEndpoint reverseClassLazyTableEndpoint, LazyOperationConfig.ReverseEngineering reverseEngineering) {
    ...
    public void initClassAnnotationPart() {
        ...
        if (reverseEngineering.isEnableSwagger()) {
            addClassAnnotationPart(String.format("@Schema(title = \"%s\",description = \"%s\")", tableEndpoint.getTableName(), tableEndpoint.getComment()));
        }
    }
    ...
    public void initClassNamePart() {
        ...
        setFileName(domain);
        addClassNamePart(String.format("public class %s {" + NormalUsedString.NEWLINE, domain) + NormalUsedString.NEWLINE);
