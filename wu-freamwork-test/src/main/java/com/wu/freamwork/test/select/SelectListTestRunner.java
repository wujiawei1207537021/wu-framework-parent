...
@Component
public class SelectListTestRunner extends AbstractSelectTestRunner {
    ...
    public void doRun() {
        ...
        List<LazyUserTest> lazyUserTests = lazyLambdaStream.selectList(basicComparison
        );
        SqlPart sqlPart = basicComparison.getSqlPart();
        String sql = sqlPart.sql();
        result.setExecuteSql(sql);
        result.setResult(lazyUserTests);
