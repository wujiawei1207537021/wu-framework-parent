...
public enum AnnotationClassEnums {
     ...
     * @return
     */
    public static ClassUo create(Class clazz) {
        final String simpleName = clazz.getSimpleName();
        final String packageName = clazz.getPackage().getName();
        final Class[] classes = clazz.getClasses();
        final List<ClassUo> includedClasses = Arrays.stream(classes).map(c -> {
            ClassUo classUo = new ClassUo();
            classUo.setName(c.getSimpleName()).setPackageName(c.getPackage().getName()).setJavaClassType(JavaClassType.CLASS).setType(LayerClass.LayerType.NULL);
            return classUo;
        }).collect(Collectors.toList());
        final ClassUo classUo = new ClassUo();

        classUo.setName(simpleName).setPackageName(packageName).setJavaClassType(JavaClassType.CLASS).setType(LayerClass.LayerType.NULL)
