...
const tempBody = $.templates('#temp_body');
const tempBodyRefModel = $.templates('#temp_body_ref_model');
const tempBodyType = $.templates('#temp_body_type');
...
// 	        url : "http://petstore.swagger.io/v2/swagger.json",
        ...
        success: function (data) {
            ...
            layui.use(['layer', 'jquery', 'element'], function () {
                const $ = layui.jquery, layer = layui.layer, element = layui.element;
            });
            const jsonData = eval(data);

            const structure = jsonData.data;
            console.log(structure);
            $("#title").html(structure.info.title);
            $("body").html($("#template").render(structure));

            $("[name='a_path']").click(function () {
                const path = $(this).attr("path");
                const method = $(this).attr("method");
                const operationId = $(this).attr("operationId");
                $.each(jsonData.paths[path], function (i, d) {
                    if (d.operationId == operationId) {
                        ...
                        if (d.responses["200"]["schema"].hasOwnProperty("type")) {
                            const model = {"type": d.responses["200"]["schema"]["type"]};
                        ...
                        //引用类型
                        const modelName = getRefName(d.responses["200"]["schema"]["$ref"]);
                        if (d.parameters) {
                            $.each(d.parameters, function (i, p) {
                                if (p["schema"]) {
                                    const parameterModelName = getRefName(p["schema"]["$ref"]);
...
function renderRefModel(domId, jsonData, modelName) {
    if (modelName) {
        const model = jsonData.definitions[modelName];
        ...
        $.each(model.properties, function (i, v) {
            if (v.items) {
                $.each(v.items, function (j, item) {
                    const typeModel = item.startsWith("#") ? getRefName(item) : item;
...
function getData(operationId) {
    let path = contextPath + $("[m_operationId='" + operationId + "']").attr("path");
    //path 参数
    $("[p_operationId='" + operationId + "'][in='path']").each(function (index, domEle) {
        const k = $(domEle).attr("name");
        const v = $(domEle).val();
        if (v) {
    ...
    //请求方式
    const parameterType = $("#content_type_" + operationId).val();
    ...
    if ("form" == parameterType) {
        ...
        const str = $("#text_tp_" + operationId).val();
        try {
...
function changeParameterType(el) {
    const operationId = $(el).attr("operationId");
...
function send(url, operationId, header, data) {
    ...
    const type = $("[m_operationId='" + operationId + "']").attr("method");

    //是否有formData类型数据
    const hasFormData = $("[p_operationId='" + operationId + "'][in='formData']").length >= 1;

    //是否有body类型数据
    const hasBody = $("[p_operationId='" + operationId + "'][in='body']").length >= 1;

    const options = {withQuotes: true};

    //发送请求
    if (hasFormData) {
        const formData = new FormData($("#form_" + operationId)[0]);
        $.ajax({
            type: type,
            url: url,
            headers: header,
    ...
    let bodyData;
    if (hasBody) {
        const dom = $("[p_operationId='" + operationId + "'][in='body']")[0];
        bodyData = $(dom).val();
    }
    const contentType = $("#consumes_" + operationId).text();
